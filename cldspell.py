...
parser.add_argument('--bal', action='store_true',  help='Balanced strategy')
...
if sum([args.aggr, args.bal, args.cons]) != 1:
    #none or more than 1 are true
    logging.error("Please provide  --aggr, --bal  or --cons")
    exit(1)
...
for line in sys.stdin:
    ...
    pred = cld3.get_language(sent)

    
    if not pred.is_reliable and (args.bal or args.cons):
        print(sent+"\tunk")
        continue

    prediction = pred.language    
